...
public class FolderObserverTest implements FolderObserver.OnFolderFileChangeListener {
    ...
    public void setUp() throws Exception {
        mTestFolder = new File(new MockContext(InstrumentationRegistry.getTargetContext()).getDir("config", 0), "observer-test");
        mTestFolder.mkdir();
    }
    ...
    public void testNotExisting() throws IOException, InterruptedException {
        Folder r = new Folder();
        r.path = new File(new MockContext(InstrumentationRegistry.getTargetContext()).getDir("config", 0), "not-existing").getPath();
