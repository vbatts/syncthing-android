...
public class SyncthingServiceTest {
    ...
    public void tearDown() {
        new File(mService.getDir("config", 0), SyncthingService.PUBLIC_KEY_FILE).delete();
    }
    ...
    public void testNotFirstStart() throws IOException {
        new File(mService.getDir("config", 0), SyncthingService.PUBLIC_KEY_FILE).createNewFile();
        Assert.assertFalse(mService.isFirstStart());
    }

    @Test
    public void testImportExportConfig() {
        File config     = new File(mService.getDir("config", 0), ConfigXml.CONFIG_FILE);
        File privateKey = new File(mService.getDir("config", 0), SyncthingService.PRIVATE_KEY_FILE);
        File publicKey  = new File(mService.getDir("config", 0), SyncthingService.PUBLIC_KEY_FILE);
