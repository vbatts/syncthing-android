...
public class SyncthingService extends Service implements
    ...
    public boolean isFirstStart() {
        return !new File(getExternalFilesDir("config"), PUBLIC_KEY_FILE).exists();
    }
    ...
    private void pollWebGui() {
        new PollWebGuiAvailableTask(getWebGuiUrl(), getExternalFilesDir("config") + "/" + HTTPS_CERT_FILE,
    ...
    public void exportConfig() {
        EXPORT_PATH.mkdirs();
        try {
            Files.copy(new File(getExternalFilesDir("config"), ConfigXml.CONFIG_FILE),
                    new File(EXPORT_PATH, ConfigXml.CONFIG_FILE));
            Files.copy(new File(getExternalFilesDir("config"), PRIVATE_KEY_FILE),
                    new File(EXPORT_PATH, PRIVATE_KEY_FILE));
            Files.copy(new File(getExternalFilesDir("config"), PUBLIC_KEY_FILE),
    ...
    public boolean importConfig() {
        ...
        try {
            Files.copy(config, new File(getExternalFilesDir("config"), ConfigXml.CONFIG_FILE));
            Files.copy(privateKey, new File(getExternalFilesDir("config"), PRIVATE_KEY_FILE));
            Files.copy(publicKey, new File(getExternalFilesDir("config"), PUBLIC_KEY_FILE));
        } catch (IOException e) {
