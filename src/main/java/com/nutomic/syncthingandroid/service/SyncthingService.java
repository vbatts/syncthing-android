...
public class SyncthingService extends Service implements
    ...
    public boolean isFirstStart() {
        return !new File(getDir("config", 0), PUBLIC_KEY_FILE).exists();
    }
    ...
    private void pollWebGui() {
        new PollWebGuiAvailableTask(getWebGuiUrl(), getDir("config", 0) + "/" + HTTPS_CERT_FILE,
    ...
    public void exportConfig() {
        EXPORT_PATH.mkdirs();
        try {
            Files.copy(new File(getDir("config", 0), ConfigXml.CONFIG_FILE),
                    new File(EXPORT_PATH, ConfigXml.CONFIG_FILE));
            Files.copy(new File(getDir("config", 0), PRIVATE_KEY_FILE),
                    new File(EXPORT_PATH, PRIVATE_KEY_FILE));
            Files.copy(new File(getDir("config", 0), PUBLIC_KEY_FILE),
    ...
    public boolean importConfig() {
        ...
        try {
            Files.copy(config, new File(getDir("config", 0), ConfigXml.CONFIG_FILE));
            Files.copy(privateKey, new File(getDir("config", 0), PRIVATE_KEY_FILE));
            Files.copy(publicKey, new File(getDir("config", 0), PUBLIC_KEY_FILE));
        } catch (IOException e) {
